...
class Profile2Controller extends Controller
{
    ...
    public function create()
    {
        ...
        return view('profile2.create', compact('categories', 'p', 'userCategory', 'active'));
    }
    ...
    public function verifyProfile()
    {
        ...
        // if ($p && $p->isVerified == 'Yes')
            // return view('profile2.verified', compact('active', 'p'));

        $countries = \Lang::get('country');

        return view('profile2.verify', compact('active', 'p', 'countries'));
    }
    ...
    public function processVerification(Request $r)
    {
        ...
        $this->validate($r, [
            ...
            'idUploadBackside' => 'required|image',
            'idUploadSelfie' => 'required|image'
        ]);

        // set id upload
        $idUpload = $r->file('idUpload');
        $idUploadBackside = $r->file('idUploadBackside');
        $idUploadSelfie = $r->file('idUploadSelfie');

        // store id picture
        $storePic = $idUpload->storePublicly('verification', env('DEFAULT_STORAGE'));
        $storePicBackside = $idUploadBackside->storePublicly('verification', env('DEFAULT_STORAGE'));
        $storePicSelfie = $idUploadSelfie->storePublicly('verification', env('DEFAULT_STORAGE'));

        // build user meta
        $userMeta['first_name'] = $r->first_name;
        $userMeta['last_name'] = $r->last_name;
        $userMeta['country'] = $r->country;
        $userMeta['city'] = $r->city;
        $userMeta['date_of_birth'] = $r->date_of_birth;
        $userMeta['passport_id_number'] = $r->passport_id_number;
        $userMeta['passport_expiry'] = $r->passport_expiry;
        $userMeta['address'] = $r->address;
        $userMeta['address2'] = $r->address2;
        $userMeta['explicit_content'] = $r->explicit_content;
        $userMeta['id'] = $storePic;
        $userMeta['id_backside'] = $storePicBackside;
        $userMeta['id_selfie'] = $storePicSelfie;
    ...
    public function setFee()
    {
        ...
        // set fee
        return view('profile2.set-fee', compact('active', 'p'));
    }
    ...
    public function accountSettings()
    {
        ...
        return view('profile2.account-settings', compact('active', 'p'));
    }
    ...
    public function mySubscribers()
    {
        ...
        return view('profile2.my-subscribers', compact('active', 'subscribers'));
    }
    ...
    public function mySubscriptions()
    {
        ...
        return view('profile2.my-subscriptions', compact('active', 'subscriptions'));
    }
