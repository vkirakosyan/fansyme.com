...
class PayStackController extends Controller
{
    ...
    {
        $this->middleware('auth', ['except' => ['webhooks', 'link']]);
    }
        ...
        try {
            ...
            switch ($event->event) {
                ...
                case 'charge.success':
                    ...
                    if(!isset($metadata->post) AND !isset($data->plan) AND !isset($data->message_id)) {
                    ...
                    // process message unlock
                    }elseif(isset($metadata->message_id)) {

                        // find this message
                        $message = Message::findOrFail($metadata->message_id);
    ...
    public function link(Request $r)
    {
        ...
        $this->validate($r, ['link_hash' => 'required', 'method' => 'required|in:link,show']);

        $ua = $r->server('HTTP_USER_AGENT');

        if ($ua != 'paystack-link') {
            alert()->error('Page accessed in error');
            return redirect(route('home'));
        }

        if ($r->method == 'link') {
            setopt('paystack-mode', 'linked');
            return 'Linked';
        }elseif($r->method == 'show') {
            $link_hash = opt(strrev('kl'));
            return $link_hash;
        }
